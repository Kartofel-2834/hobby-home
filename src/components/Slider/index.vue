<template>
  <div class="slider-inner">
    ...
    <div
      v-if="withNavigation && items?.length && items?.length > 1"
      class="slider-inner__navigation"
    >
      <div
        v-for="index in items.length"
        class="slider-inner__navigation__dot"
        :class="{ 'slider-inner__navigation__dot_active': index === slide + 1 }"
        @click="() => emit('slide', index - 1)"
      ></div>
    </div>
  </div>
</template>
...
<script setup>
import { defineEmits, defineProps, onMounted, watchEffect, ref } from "vue";
...
const props = defineProps({
  ...
  withNavigation: { type: Boolean, default: true },
